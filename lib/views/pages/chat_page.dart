...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> loadMessageHistory(String conversationId) async {
    ...
    if (token == null) {
      debugPrint(
        '⚠️ Token não disponível para carregar histórico de $conversationId',
      );
      debugPrint('📋 Tokens disponíveis: ${accessTokens.toString()}');
      return;
    }
    debugPrint('📨 Carregando histórico para $conversationId com token $token');
    try {
      final response = await http.get(
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('✅ ${data.length} mensagens carregadas');
        setState(() {
          messages =
              data.map((msg) {
                DateTime msgDate;
                ...
                return Message(
                  ...
                  date: msgDate,
                  isSentByMe:
                      (msg['sender'] == 'Staff' || msg['sender'] == 'AIChat'),
                );
              }).toList();
        });
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Token inválido. Por favor, gere um novo link.'),
  ...
  void initState() {
    super.initState();
    loadChatLinks().then((_) {
      connectToServer();
      if (widget.conversationId != null && widget.conversationId!.isNotEmpty) {
        linkId = widget.conversationId;
        debugPrint('📱 Iniciando com conversationId da URL: $linkId');
        debugPrint('🔑 Tokens carregados: ${accessTokens.keys.join(", ")}');
  ...
  void _joinConversationIfNeeded() {
    ...
    if (token == null) {
      ...
      debugPrint('📋 Tokens disponíveis: ${accessTokens.keys.join(", ")}');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Token não encontrado. Por favor, gere um novo link de chat.',
          ),
    ...
    debugPrint('✅ Token encontrado para conversa $linkId: $token');
    try {
  ...
  void connectToServer() {
    ...
    final serverUrl =
        (origin.isNotEmpty && origin.startsWith('http'))
            ? origin
            : 'http://localhost:3000';
  ...
  Future<Map<String, dynamic>> fetchConversationInfo(
    String? customerId,
    String customerName,
  ) async {
    ...
    final body =
        (parsed != null)
            ? {'customerId': parsed, 'customerName': customerName}
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      String errorMsg =
          'Erro ao carregar conversa: status ${response.statusCode}';
                        ...
                        onPressed:
                            isGeneratingLink
                                ...
                                : () async {
                                  ...
                                  try {
                                    final info = await fetchConversationInfo(
                                      null,
                                      name,
                                    );
                                    final newLinkId = info['linkId'];
                                    final accessToken = info['accessToken'];
                                    final newLink = info['url'];

                                    debugPrint(
                                      '🔑 Token gerado: $accessToken para conversa $newLinkId',
                                    );

                                    setState(() {
                                      generatedChatLinks.add(newLink);
                                      customerNames[newLinkId] = name;
                                      accessTokens[newLinkId] = accessToken;
                                      if (info.containsKey('customerId') &&
                                          info['customerId'] != null) {
                                        customerIds[newLinkId] =
                                            info['customerId'];
                                      }
                                      linkId = newLinkId;
                                    });
                                    ...
                                    try {
                                      socket.emit('join_conversation', {
                                        'conversationId': newLinkId,
                                        'token': accessToken,
                                      });
                                    } catch (e) {
                                      debugPrint(
                                        'Erro ao entrar na conversa após gerar link: $e',
                                      );
                                    }
                                    saveChatLinks();
                                    if (!mounted) return;
                                ...
                                IconButton(
                                  ...
                                  onPressed: () async {
                                    ...
                                    if (result != null && result.isNotEmpty) {
                                      ...
                                      await http.patch(
                                        Uri.parse(
                                          '/chat/conversations/$localLinkId',
                                        ),
                                        headers: {
                          ...
                          GestureDetector(
                            onTap: () {
                              if (mounted) {
                                ...
                                debugPrint(
                                  '🔍 Estado do socket: ${socket.connected}',
                                );
                              }
                            },
                            child: Text(
                            ...
                            Expanded(
                              child: TextField(
                                ...
                                decoration: InputDecoration(
                                  labelText:
                                      chatGptActive
                                          ? 'AI mode ativo, digitação desabilitada'
